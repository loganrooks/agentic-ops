...
>
> Updated 2026-05-14: TS-stack rows (prix-guesser, epistemic-agency) carry `Bash(tsc --noEmit *)` only, no `Bash(eslint:*)`. ESLint loads project-level JavaScript config (`eslint.config.js`, `.eslintrc.js`) from the PR head as executable JavaScript — the threat class [ADR-004](../../docs/adr/ADR-004-allowlist-policy.md) §"Why no test execution" excludes; reclassified from §Acceptable to Forbidden by [ADR-010](../../docs/adr/ADR-010-static-analysis-only-clarification.md) §Decision §1. Caught by Codex P1 review on prix-guesser PR #1 (consumer repo); carried forward to the still-vulnerable epistemic-agency row before /goal reaches it. prix-guesser's `Bash(tsc:*)` also tightened to `Bash(tsc --noEmit *)` (Claude Code permission space-form per ADR-004; the colon-form would let `tsc --build`/`tsc --emit` slip through). Known limitation: `Bash(tsc --noEmit *)` remains argument-injection-defeatable (`tsc --noEmit false a.ts` emits a.js); blast radius is academic at the review path (emit to ephemeral workspace, no subsequent step reads or executes the output) per [ADR-010](../../docs/adr/ADR-010-static-analysis-only-clarification.md) §Decision §2.
