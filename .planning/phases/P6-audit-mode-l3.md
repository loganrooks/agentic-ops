...
- **Widen the trusted-actor `if:` clause to admit the new L3 audit triggers on non-PR issues.** The existing P3 widening (review.yml lines 132–141) only allows non-PR issue comments for the bare `audit` trigger family: `@claude audit` (bare), `@claude audit` followed by space, `@claude audit:` (with lens), and `@claude audit` followed by a newline. Without extending it, `@claude audit-matrix:<lens>` and `@claude audit-all` posted on a plain issue (the canonical audit-on-main path) would be rejected before reaching the dispatcher. Add equivalent branches for the `audit-matrix` and `audit-all` trigger families (with the same four shapes per family: bare, space-suffix, colon-suffix where applicable, newline-suffix) to the issue-comment-only branch of the `if:` clause.
