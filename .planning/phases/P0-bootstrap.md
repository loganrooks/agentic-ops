...
- The agentic-ops repo root exists locally and is a clean git working tree (the absolute path is environment-specific and captured in `auto-execution/STATE.md` notes at bootstrap time)
- The source plan exists at `~/.claude/plans/<plan-id>.md` where `<plan-id>` is recorded in STATE.md's `Plan file` field
