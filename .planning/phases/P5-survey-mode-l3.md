...
- Document the rule in an ADR-002 update.
- Time: ~15 min.
...
### P5-T7 — Update ADR-002 for L3 implementation
- Append an "Implementation notes" section: 3-job structure, threshold input, partial-failure semantics.
- Cross-link from the ADR back to `phases/P5-survey-mode-l3.md`.
- Note deviations from the original ADR (e.g. `max-parallel` choice, ≤8 zone cap).
- Keep the ADR diff in the same PR as the refactor.
- Time: ~15 min.
...
- Tag the maintainer; reference the ADR-002 update.
- Time: ~10 min.
